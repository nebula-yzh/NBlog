...
public class BlogServiceImpl implements BlogService {
    ...
    private void setBlogViewsFromRedisToPageResult(PageResult<BlogInfo> pageResult) {
        ...
        for (int i = 0; i < blogInfos.size(); i++) {
            ...
            Object viewObj = redisService.getValueByHashKey(redisKey, blogId);
            int view = 0;
            //进行判空，redis中是否有博客浏览量的键
            //一般情况在项目启动已将博客浏览量更新到redis，除非后面又清空了redis
            if (!StringUtils.isEmpty(viewObj)) {
                view = (int) viewObj;
            }
